...
const SkillsSection = () => {
  ...
  // Kategori Listesini Filtrele (Mobilde başlangıçta sadece ilk 2)
  const displayedCategories = isMobile && !showAllSkills 
    ? SKILLS_CATEGORY.slice(0, 2) 
    : SKILLS_CATEGORY;
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ y }} className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        />
        <div 
          className={`absolute bottom-40 left-1/4 w-64 h-96 rounded-full blur-3xl opacity-5 ${
      ...
      <div className="max-w-6xl mx-auto relative z-10">
                  ...
                  {category.skills.map((skill) => (
                    <div key={skill.name} className="group">
                      ...
                      >
                        ...
                        >
                          <div className="absolute inset-0 bg-white/20 rounded-fill opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        </motion.div>
                      </div>
                    </div>
                  ))}
        ...
        {/* Additional Skills - Sadece desktop'ta veya mobilde "Show More" tıklandığında görünür */}
        {(!isMobile || showAllSkills) && (
          <motion.div
            initial="hidden"
            animate={isInView ? "visible" : "hidden"}
